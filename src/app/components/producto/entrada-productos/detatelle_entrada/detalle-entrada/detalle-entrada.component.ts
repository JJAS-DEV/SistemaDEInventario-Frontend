...
export class DetalleEntradaComponent implements OnInit {
  ...
  buscarProducto(idBuscado: number | undefined): number | undefined {
    return this.listaNumerosDiferentes.find(producto => producto.id === idBuscado)?.cantidad;


  }
  getCantidadProducto(productoId: number | undefined): number {
    return this.buscarProducto(productoId) ?? 0;
  }
